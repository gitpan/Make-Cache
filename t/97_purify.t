#!/usr/bin/perl -w
#$Id: 97_purify.t 20789 2006-05-24 17:10:53Z wsnyder $
######################################################################
...
our $ObjCache = "$PERL ../objcache --read --write --wrapper";  #--debug
our $TestId = 96;
...
for (my $i=0; $i<2; $i++) {
    ...
    my $oc_out = run_qx("$ObjCache t/fake_purify g++ -DIGNORED test1.cpp -c -o test1.o");
    ok($oc_out);
...
for (my $i=0; $i<2; $i++) {
    ...
    my $oc_out = run_qx("$ObjCache t/fake_purify g++ -DDIFFIGNORED test1.cpp -c -o test1.o");
    ok($oc_out);
